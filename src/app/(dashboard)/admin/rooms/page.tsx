...
// 🚀 IMPORTAMOS NUESTRO MOTOR DE ALERTAS PREMIUM
...
import { Bed, Plus, ArrowLeft, Building2, ChevronLeft, ChevronRight, MapPin, Filter, Image as ImageIcon, MoreVertical, Edit, Activity, Search, Check, X, CalendarCheck, UserCheck } from "lucide-react";
...
function RoomsContent() {
  ...
  const handleCreateRoom = async (e: React.FormEvent<HTMLFormElement>) => { 
    ...
    try { 
      ...
      Alerts.toast("Room created successfully");
    } catch (err) { 
      Alerts.error("Creation Failed", "Could not register the new room."); 
    } finally { 
      setIsCreating(false); 
    } 
  };
  ...
  const handleUpdateRoom = async (e: React.FormEvent<HTMLFormElement>) => { 
    ...
    try { 
      ...
      fetchRoomsByHotel(selectedHotel.id, currentPage); 
      Alerts.toast("Room updated successfully");
    } catch (err) { 
      Alerts.error("Update Failed", "Could not mutate room metrics."); 
    } finally { 
      setIsUpdating(false); 
    } 
  };
  ...
  const handleStatusChange = async (roomId: number, newStatus: string) => { 
    ...
    try { 
      ...
      fetchRoomsByHotel(selectedHotel.id, currentPage); 
      Alerts.toast(`Status changed to ${newStatus}`);
    } catch (err) { 
      Alerts.error("Status Update Failed", "Could not mutate operational status."); 
    } 
  };

  /**
   * 🚀 Cross-Entity Transaction: Processes a Reservation via the Room interface using SweetAlert.
   */
  const handleBookRoom = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!selectedUser || !roomToBook) { 
      Alerts.error("Missing Identity", "Please select a guest profile layout before proceeding."); 
      return; 
    }
    ...
    try {
      ...
      Alerts.success("Reservation Secured", "Ledger Window Fixed: Reservation processed successfully."); 
      fetchRoomsByHotel(selectedHotel!.id, currentPage);
    } catch (err) { 
      Alerts.error("Boundary Blocked", "Overbooking detected or validation failed. Please review dates."); 
    } finally { 
      setIsBooking(false); 
    }
  };

  // 🚀 RESTAURACIÓN DE LA PAGINACIÓN Y CONTROLADORES DE MODALES
  const openBookModal = (room: Room) => { 
    if(room.roomStatus === 'MAINTENANCE') { 
      Alerts.error("Asset Locked", "This unit is currently flagged for maintenance operations."); 
      return; 
    } 
    setRoomToBook(room); 
  ...
  return (
    <div className="p-8 space-y-6 flex flex-col min-h-[calc(100vh-4rem)] max-w-[1600px] mx-auto w-full animate-in fade-in zoom-in-95 duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-border/50 pb-6 gap-4">
        ...
        <div className="flex items-center gap-3">
          <Select value={filterStatus} onValueChange={(val) => { setFilterStatus(val); setCurrentPage(0); }}>
            <SelectTrigger className="w-[180px] bg-background/50 border-border/50 font-medium shadow-sm hover:border-primary transition-colors cursor-pointer h-10">
              ...
              <DialogContent className="sm:max-w-[500px] md:max-w-[580px] border-border/50 bg-card/95 backdrop-blur-xl p-8 shadow-2xl rounded-md ring-1 ring-black/10">
                ...
                <form onSubmit={handleCreateRoom} className="space-y-4 mt-6">
                  ...
                  <div className="flex justify-end gap-3 pt-4 border-t border-border/40 mt-8">
                    ...
                    <Button type="submit" className="rounded-md font-bold cursor-pointer bg-primary text-primary-foreground shadow-lg h-11 px-8 hover:-translate-y-0.5 transition-transform" disabled={isCreating}>Deploy Room Unit</Button>
                  </div>
                </form>
              </DialogContent>
            </Dialog>
      ...
      <div className="flex-1 flex flex-col justify-between space-y-6">
                    ...
                    {currentUser?.role === 'ADMIN' && (
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="border border-border/50 bg-card/95 backdrop-blur-xl shadow-xl p-1">
                          ...
                          <DropdownMenuItem onClick={(e) => openEditModal(room, e as unknown as React.MouseEvent)} className="font-medium cursor-pointer hover:bg-primary/10 transition-colors rounded-sm"><Edit className="mr-2 h-4 w-4 text-primary" /> Edit Room Details</DropdownMenuItem>
                          <DropdownMenuSeparator />
                          ...
                          <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleStatusChange(room.id, "AVAILABLE"); }} className="rounded-sm font-bold cursor-pointer text-emerald-500 hover:bg-emerald-500/10 transition-colors"><Activity className="mr-2 h-4 w-4" /> Set Available</DropdownMenuItem>
                          <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleStatusChange(room.id, "MAINTENANCE"); }} className="rounded-sm font-bold cursor-pointer text-destructive hover:bg-destructive/10 transition-colors"><Activity className="mr-2 h-4 w-4" /> Set Maintenance</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    )}
                  </td>
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-card/40 backdrop-blur-md border shadow-sm border-border/50 mt-auto dark:ring-1 dark:ring-white/10 rounded-md"><span className="text-sm text-muted-foreground font-medium">Page <span className="font-black text-foreground">{currentPage + 1}</span> of <span className="font-black text-foreground">{totalPages}</span> ({totalElements} total rooms)</span><div className="flex items-center gap-2"><Button variant="outline" size="sm" onClick={handlePreviousPage} disabled={currentPage === 0} className="font-bold hover:bg-accent/50 cursor-pointer"><ChevronLeft className="w-4 h-4 mr-1" /> Prev</Button><Button variant="outline" size="sm" onClick={handleNextPage} disabled={currentPage >= totalPages - 1} className="font-bold hover:bg-accent/50 cursor-pointer">Next <ChevronRight className="w-4 h-4 ml-1" /></Button></div></div>
      </div>

      {/* EDIT MODAL */}
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[500px] md:max-w-[580px] border-border/50 bg-card/95 backdrop-blur-xl p-8 shadow-2xl rounded-md ring-1 ring-black/10">
          <DialogHeader><DialogTitle className="text-2xl font-black tracking-tight">Edit Room Metrics</DialogTitle><DialogDescription className="text-sm mt-1">Modify architectural indices for Unit {roomToEdit?.roomNumber}</DialogDescription></DialogHeader>
          {roomToEdit && (
      ...
      {/* 🚀 MODAL DE RESERVA (BOOKING) - REFACTORIZADO A NIVEL ENTERPRISE CON SWEET ALERTS */}
      <Dialog open={isBookModalOpen} onOpenChange={(open) => { setIsBookModalOpen(open); if (!open) { setSelectedUser(null); setUserSearchQuery(""); }}}>
        <DialogContent className="sm:max-w-[550px] md:max-w-[650px] border-border/40 bg-card/95 backdrop-blur-2xl p-8 shadow-2xl rounded-md ring-1 ring-black/10 dark:ring-white/10 animate-in zoom-in-95 duration-200">
          ...
          <DialogHeader className="mb-2">
            ...
            <DialogDescription className="text-sm mt-1 font-medium text-muted-foreground flex items-center gap-2">
              Assigning Guest Clearance to <span className="inline-flex px-2 py-0.5 rounded border border-primary/20 bg-primary/10 text-primary font-black uppercase text-xs shadow-sm">Unit {roomToBook?.roomNumber}</span>
            </DialogDescription>
          </DialogHeader>

          <form onSubmit={handleBookRoom} className="space-y-6 mt-6">
            ...
            <div className="space-y-2 relative z-50">
              ...
              {!selectedUser ? (
                ...
                /* 🚀 LA TARJETA HERO DEL USUARIO SELECCIONADO ESTÁ DE VUELTA Y MÁS ELEGANTE */
            ...
            <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Number of Active Guests</Label><Input name="numberOfGuests" type="number" min={1} max={roomToBook?.capacity || 10} required className="bg-background h-12 shadow-inner" placeholder={`Max capacity: ${roomToBook?.capacity} guests`} /></div>
            
            <div className="flex justify-end gap-3 pt-6 border-t border-border/40 mt-8">
              <Button type="button" variant="ghost" className="rounded-md font-bold h-12 px-6 cursor-pointer" onClick={() => setIsBookModalOpen(false)}>Cancel Window</Button>
              <Button type="submit" className="rounded-md font-bold cursor-pointer shadow-xl hover:-translate-y-0.5 active:translate-y-0 transition-all h-12 px-8 bg-primary text-primary-foreground text-sm" disabled={isBooking}>{isBooking ? "Validating Ledger Token..." : "Lock Secure Booking"}</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
