...
function RoomsContent() {
  ...
  const handleCreateRoom = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try { await api.post("/rooms", { roomNumber: formData.get("roomNumber"), capacity: Number(formData.get("capacity")), pricePerNight: Number(formData.get("pricePerNight")), roomType, hotelId: selectedHotel.id }); setIsCreateModalOpen(false); fetchRoomsByHotel(selectedHotel.id, currentPage); } catch (err) { alert("Error creating room."); } finally { setIsCreating(false); }
  };
  ...
  const handleUpdateRoom = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try { await api.put(`/rooms/${roomToEdit.id}`, { roomNumber: formData.get("roomNumber"), capacity: Number(formData.get("capacity")), pricePerNight: Number(formData.get("pricePerNight")), roomType: formData.get("roomType"), hotelId: selectedHotel.id }); setIsEditModalOpen(false); fetchRoomsByHotel(selectedHotel.id, currentPage); } catch (err) { alert("Error updating room."); } finally { setIsUpdating(false); }
  };
    ...
    <div className="p-8 space-y-6 flex flex-col min-h-[calc(100vh-4rem)] max-w-[1600px] mx-auto w-full animate-in fade-in zoom-in-95 duration-300">
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-border/50 pb-6 gap-4">
        <div className="space-y-1">
          <Button variant="ghost" size="sm" onClick={handleBackToHotels} className="mb-2 -ml-2 text-muted-foreground hover:text-foreground hover:bg-accent/50 transition-colors cursor-pointer"><ArrowLeft className="w-4 h-4 mr-2" /> Back to Directory</Button>
        ...
        <div className="flex items-center gap-3">
          ...
          {currentUser?.role === 'ADMIN' && (
            <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
              <DialogTrigger asChild><Button className="flex items-center gap-2 font-bold shadow-sm hover:-translate-y-0.5 hover:shadow-lg transition-all cursor-pointer"><Plus className="w-4 h-4" /> Add Room</Button></DialogTrigger>
              <DialogContent className="sm:max-w-[425px] border-border/50 bg-card/95 backdrop-blur-xl shadow-2xl">
                <DialogHeader><DialogTitle>Register New Room</DialogTitle><DialogDescription>Adding capacity to <span className="font-bold text-primary">{selectedHotel?.name}</span>.</DialogDescription></DialogHeader>
                <form onSubmit={handleCreateRoom} className="space-y-4 mt-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Room Number</Label><Input name="roomNumber" required className="bg-background shadow-inner" /></div>
                    <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Capacity</Label><Input name="capacity" type="number" min={1} required className="bg-background shadow-inner" /></div>
                  </div>
                  <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Price Per Night</Label><Input name="pricePerNight" type="number" step="0.01" min={1} required className="bg-background shadow-inner" /></div>
                  <div className="space-y-1.5">
                    ...
                    <Select onValueChange={setRoomType} value={roomType}>
                      <SelectTrigger className="bg-background border-border/50 shadow-inner cursor-pointer"><SelectValue /></SelectTrigger>
                      <SelectContent className="border-border/50 bg-card/95 backdrop-blur-xl">
                  ...
                  <Button type="submit" className="w-full mt-6 font-bold shadow-md hover:-translate-y-0.5 transition-transform cursor-pointer" disabled={isCreating}>{isCreating ? "Processing..." : "Create Room"}</Button>
                </form>
              </DialogContent>
            </Dialog>
          )}
      ...
      <div className="flex-1 flex flex-col justify-between space-y-6">
        <div className="bg-card/40 backdrop-blur-md border border-border/50 rounded-md shadow-sm overflow-hidden dark:ring-1 dark:ring-white/10">
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="bg-muted/20 text-muted-foreground border-b border-border/50">
              <tr><th className="px-6 py-4 font-bold uppercase tracking-wider text-[10px]">Room #</th><th className="px-6 py-4 font-bold uppercase tracking-wider text-[10px]">Category</th><th className="px-6 py-4 font-bold uppercase tracking-wider text-[10px]">Capacity</th><th className="px-6 py-4 font-bold uppercase tracking-wider text-[10px]">Status</th><th className="px-6 py-4 font-bold uppercase tracking-wider text-[10px] text-right">Price/Night</th><th className="px-6 py-4 font-bold uppercase tracking-wider text-[10px] text-right">Manage</th></tr>
            </thead>
              ...
              {rooms.map((room) => (
                <tr key={room.id} className="hover:bg-accent/20 hover:shadow-sm transition-all group">
                  ...
                  <td className="px-6 py-4 text-right">
                    {currentUser?.role === 'ADMIN' && (
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild><Button variant="ghost" className="h-8 w-8 p-0 hover:bg-accent/50 cursor-pointer hover:shadow-md transition-all"><MoreVertical className="h-4 w-4 text-muted-foreground group-hover:text-foreground" /></Button></DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="border border-border/50 bg-card/95 backdrop-blur-xl shadow-xl p-1">
                          ...
                          <DropdownMenuItem onClick={() => openEditModal(room)} className="font-medium cursor-pointer hover:bg-primary/10 transition-colors"><Edit className="mr-2 h-4 w-4 text-primary" /> Edit Room Details</DropdownMenuItem>
                          <DropdownMenuSeparator />
                          ...
                          <DropdownMenuItem onClick={() => handleStatusChange(room.id, "AVAILABLE")} className="font-bold cursor-pointer text-emerald-500 hover:bg-emerald-500/10 transition-colors"><Activity className="mr-2 h-4 w-4" /> Set Available</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleStatusChange(room.id, "MAINTENANCE")} className="font-bold cursor-pointer text-destructive hover:bg-destructive/10 transition-colors"><Activity className="mr-2 h-4 w-4" /> Set Maintenance</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    )}
                  </td>
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-card/40 backdrop-blur-md border rounded-md shadow-sm border-border/50 mt-auto dark:ring-1 dark:ring-white/10"><span className="text-sm text-muted-foreground font-medium">Page <span className="font-black text-foreground">{currentPage + 1}</span> of <span className="font-black text-foreground">{totalPages}</span> ({totalElements} total rooms)</span><div className="flex items-center gap-2"><Button variant="outline" size="sm" onClick={handlePreviousPage} disabled={currentPage === 0} className="font-bold hover:bg-accent/50 cursor-pointer"><ChevronLeft className="w-4 h-4 mr-1" /> Prev</Button><Button variant="outline" size="sm" onClick={handleNextPage} disabled={currentPage >= totalPages - 1} className="font-bold hover:bg-accent/50 cursor-pointer">Next <ChevronRight className="w-4 h-4 ml-1" /></Button></div></div>
      </div>
        ...
        <DialogContent className="sm:max-w-[425px] border-border/50 bg-card/95 backdrop-blur-xl shadow-2xl">
          ...
          {roomToEdit && (
            <form onSubmit={handleUpdateRoom} className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Room Number</Label><Input name="roomNumber" defaultValue={roomToEdit.roomNumber} required className="bg-background shadow-inner" /></div>
                <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Capacity</Label><Input name="capacity" defaultValue={roomToEdit.capacity} type="number" min={1} required className="bg-background shadow-inner" /></div>
              </div>
              <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Price Per Night (USD)</Label><Input name="pricePerNight" defaultValue={roomToEdit.pricePerNight} type="number" step="0.01" min={1} required className="bg-background shadow-inner" /></div>
              <div className="space-y-1.5">
                <Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Classification</Label>
              ...
              <Button type="submit" className="w-full mt-6 font-bold shadow-md hover:-translate-y-0.5 transition-transform cursor-pointer" disabled={isUpdating}>{isUpdating ? "Processing..." : "Save Changes"}</Button>
            </form>
          )}
        </DialogContent>
      </Dialog>
