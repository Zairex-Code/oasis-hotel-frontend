 ...
 * Features real-time operational status toggling and dynamic search filtering against the Spring Boot backend.
 */
...
export default function HotelsPage() {
  ...
  // Local State Management for Entity Persistence
  ...
  /**
   * Fetches hotel entities based on active filters and pagination states.
  ...
  // Lifecycle Hook: Triggers refetch on filter or pagination changes with a soft debounce (300ms)
  ...
  /**
   * Handles the creation of a new Hotel entity.
   * Utilizes e.preventDefault() to block standard HTML form submission, preventing Next.js router reloads.
   */
  ...
  /**
   * Handles the mutation of an existing Hotel entity.
   */
  ...
  /**
   * Dispatches a direct PUT request to toggle the operational status of the branch.
   */
  ...
  return (
    <div className="p-8 space-y-6 flex flex-col min-h-[calc(100vh-4rem)] max-w-[1600px] mx-auto w-full animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between border-b border-border/50 pb-6">
        <div><h1 className="text-3xl font-black tracking-tight flex items-center gap-3"><Building2 className="w-8 h-8 text-primary" /> Ecosystem Branches</h1><p className="text-muted-foreground font-medium text-sm mt-1">Audit, structure, and filter global hotel infrastructure footprints.</p></div>
        {['ADMIN', 'HOTEL_MANAGER'].includes(user?.role || '') && (
          <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
            <DialogTrigger asChild><Button className="flex items-center gap-2 rounded-md font-bold shadow-md shadow-primary/10 hover:shadow-lg hover:-translate-y-0.5 transition-all cursor-pointer h-10 px-5"><Plus className="w-4 h-4" /> Add New Hotel</Button></DialogTrigger>
            <DialogContent className="sm:max-w-[550px] md:max-w-[620px] rounded-md border-border/40 bg-card/95 backdrop-blur-xl p-8 shadow-2xl ring-1 ring-black/10 dark:ring-white/10">
              <DialogHeader><DialogTitle className="text-2xl font-black tracking-tight flex items-center gap-2"><Sparkles className="w-5 h-5 text-primary animate-pulse" /> Register Luxury Branch</DialogTitle><DialogDescription className="text-sm text-muted-foreground font-medium mt-1">Inject a high-end hospitality asset into the secure network ledger.</DialogDescription></DialogHeader>
              <form onSubmit={handleCreateHotel} className="space-y-5 mt-6">
                ...
                <div className="grid grid-cols-2 gap-4"><div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-muted-foreground tracking-wider">Target City</Label><Input name="city" required placeholder="e.g., Miami" className="bg-background/40 h-11" /></div><div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-muted-foreground tracking-wider">Stars Rating</Label><Input name="stars" type="number" min="1" max="5" placeholder="5" required className="bg-background/40 h-11" /></div></div>
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-border/40 mt-8"><Button type="button" variant="ghost" className="rounded-md font-bold cursor-pointer h-11 px-6" onClick={() => setIsCreateModalOpen(false)}>Cancel</Button><Button type="submit" className="rounded-md font-bold cursor-pointer shadow-lg shadow-primary/20 h-11 px-8 hover:-translate-y-0.5 transition-transform" disabled={isCreating}>{isCreating ? "Saving Entry Asset..." : "Deploy Asset Channel"}</Button></div>
              </form>
            </DialogContent>
          </Dialog>
        )}
      ...
      {isLoading ? <div className="py-12 text-center text-muted-foreground animate-pulse font-bold flex-1">Querying database registers...</div> : (
        <div className="flex-1 flex flex-col justify-between space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {hotels.map((hotel) => (
              <Card key={hotel.id} onClick={() => openHotelDetails(hotel)} className="overflow-hidden border border-border/50 bg-card/40 backdrop-blur-md shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 rounded-md flex flex-col justify-between group cursor-pointer dark:hover:ring-1 dark:hover:ring-white/10">
        ...
        <DialogContent className="sm:max-w-[550px] md:max-w-[600px] rounded-md border-border/40 bg-card/95 backdrop-blur-xl p-8 shadow-2xl ring-1 ring-black/10 dark:ring-white/10">
          ...
          {hotelToEdit && (
            <form onSubmit={handleUpdateHotel} className="space-y-4 mt-6">
              <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-muted-foreground tracking-wider">Hotel Identity Name</Label><Input name="name" defaultValue={hotelToEdit.name} required className="bg-background/40 h-11" /></div>
              <div className="grid grid-cols-2 gap-4"><div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-muted-foreground tracking-wider">City Coordinates</Label><Input name="city" defaultValue={hotelToEdit.city} required className="bg-background/40 h-11" /></div><div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-muted-foreground tracking-wider">Stars Rank (1-5)</Label><Input name="stars" type="number" min="1" max="5" defaultValue={hotelToEdit.stars} required className="bg-background/40 h-11" /></div></div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-border/40 mt-8"><Button type="button" variant="ghost" className="rounded-md font-bold h-11 px-6 cursor-pointer" onClick={() => { setIsEditModalOpen(false); setHotelToEdit(null); }}>Cancel</Button><Button type="submit" className="rounded-md font-bold cursor-pointer shadow-md hover:-translate-y-0.5 transition-transform h-11 px-8" disabled={isUpdating}>{isUpdating ? "Processing Mutations..." : "Commit Secure Overwrite"}</Button></div>
            </form>
          )}
        </DialogContent>
      </Dialog>
