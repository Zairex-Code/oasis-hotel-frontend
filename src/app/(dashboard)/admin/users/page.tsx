...
export default function UsersPage() {
  ...
  // 🚀 Sincronización con el Backend usando Filtros
  const fetchUsers = async (pageToFetch: number = 0) => {
    try {
      setIsLoading(true);
      let endpoint = `/users?page=${pageToFetch}`;

      // Enrutamiento inteligente a los endpoints de Java según el filtro activo
      if (filterName.trim() !== "") {
  ...
  const handleCreateUser = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    } catch (err: any) { alert("Failed to inject user entity. Review backend logs."); } finally { setIsCreating(false); }
  };

  // 🚀 NUEVO: MUTACIÓN: EDITAR USUARIO
  const handleUpdateUser = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // 1. Actualizamos la información básica (Nombre)
      await api.put(`/users/${userToEdit.id}`, {
        firstName: formData.get("firstName"),
        lastName: formData.get("lastName")
      });

      // 2. Si el rol cambió y el usuario es ADMIN, actualizamos el rol en el endpoint dedicado
      ...
      alert("Failed to update identity parameters.");
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  return (
    <div className="p-8 space-y-6 flex flex-col min-h-[calc(100vh-4rem)] max-w-[1600px] mx-auto w-full animate-in fade-in duration-300">
        ...
        <div className="flex items-center gap-3">
            ...
            <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
                ...
                <form onSubmit={handleCreateUser} className="space-y-4 mt-4">
                    ...
                    <div className="space-y-1.5"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">First Name</Label><Input name="firstName" required className="rounded-md bg-background" /></div>
                    <div className="space-y-1.5"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Last Name</Label><Input name="lastName" required className="rounded-md bg-background" /></div>
                    </div>
                    <div className="space-y-1.5"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Email Coordinate</Label><Input name="email" type="email" required className="rounded-md bg-background" /></div>
                    <div className="space-y-1.5"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Temporary Password</Label><Input name="password" type="password" required className="rounded-md bg-background" /></div>
                    ...
                    <Button type="submit" className="w-full mt-6 rounded-md font-bold" disabled={isCreating}>{isCreating ? "Processing..." : "Generate Identity"}</Button>
                </form>
                </DialogContent>
            </Dialog>
            )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-md bg-card/40 backdrop-blur-md border border-border/50 shadow-sm">
        <div className="space-y-1.5">
          ...
          <Input placeholder="Search user by first or last name..." value={filterName} onChange={(e) => { setFilterRole("ALL"); setFilterName(e.target.value); setCurrentPage(0); }} className="rounded-md bg-background/50 border-border/50 text-sm" />
        </div>
        <div className="space-y-1.5">
          <Label className="text-xs font-bold text-muted-foreground uppercase flex items-center gap-1"><Filter className="w-3 h-3 text-chart-2" /> Clearance Level</Label>
          <Select value={filterRole} onValueChange={(val) => { setFilterName(""); setFilterRole(val); setCurrentPage(0); }}>
            ...
            <SelectContent className="rounded-md border-border/50 bg-card/95 backdrop-blur-xl">
              ...
              <SelectItem value="HOTEL_MANAGER">Branch Managers</SelectItem>
              ...
              <tbody className="divide-y divide-border/30">
                {users.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-12 text-center font-medium text-muted-foreground">No identities match your search criteria.</td></tr>
                    ...
                    users.map((userObj) => (
                    <tr key={userObj.id} className="hover:bg-accent/20 transition-colors group">
                        <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                            <div className="flex items-center justify-center w-10 h-10 rounded-md bg-primary/10 text-primary font-black text-sm uppercase border border-primary/20 shadow-sm group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
                        ...
                        <td className="px-6 py-4">
                        <span className={`inline-flex px-3 py-1 rounded-md text-[10px] uppercase tracking-widest font-black border ${
                        ...
                        {currentUser?.role === 'ADMIN' && (
                            ...
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" className="h-8 w-8 p-0 rounded-md hover:bg-accent/50"><MoreHorizontal className="h-4 w-4 text-muted-foreground" /></Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="rounded-md bg-card/95 backdrop-blur-xl border-border/50 shadow-xl">
                                <DropdownMenuLabel className="font-bold text-xs uppercase tracking-widest text-muted-foreground">Scope Actions</DropdownMenuLabel>
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onClick={() => openEditModal(userObj)} className="rounded-md font-medium cursor-pointer">
                                    <Edit className="mr-2 h-4 w-4 text-primary" /> Edit Profile Details
                                </DropdownMenuItem>
                                {userObj.role !== 'ADMIN' && (
                                    <DropdownMenuItem className="text-destructive focus:text-destructive rounded-md cursor-pointer font-bold mt-1">
            ...
            <form onSubmit={handleUpdateUser} className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input name="firstName" defaultValue={userToEdit.firstName} required className="rounded-md bg-background" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Last Name</Label>
                  <Input name="lastName" defaultValue={userToEdit.lastName} required className="rounded-md bg-background" />
                </div>
              </div>
              <div className="space-y-1.5 opacity-50 cursor-not-allowed">
                <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Email Coordinate (Immutable)</Label>
                <Input value={userToEdit.email} readOnly className="rounded-md bg-background" />
              </div>
              
              {/* Only ADMINs can change roles, and they shouldn't demote themselves easily */}
              {currentUser?.role === 'ADMIN' && currentUser.id !== userToEdit.id && (
                <div className="space-y-1.5">
                  <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">System Role Clearance</Label>
              ...
              <Button type="submit" className="w-full mt-6 rounded-md font-bold" disabled={isUpdating}>
