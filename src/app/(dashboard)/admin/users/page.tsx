...
export default function UsersPage() {
  ...
  const handleUpdateUser = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault(); if (!userToEdit) return; setIsUpdating(true); const formData = new FormData(e.currentTarget);
    try {
      await api.put(`/users/${userToEdit.id}`, { firstName: formData.get("firstName"), lastName: formData.get("lastName") });
      if (editRole !== userToEdit.role && currentUser?.role === 'ADMIN') await api.put(`/users/set-role/${userToEdit.id}`, { role: editRole });
      setIsEditModalOpen(false); setUserToEdit(null); fetchUsers(currentPage);
    } catch (err) { alert("Failed to update identity."); } finally { setIsUpdating(false); }
  };
            ...
            <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
                ...
                <form onSubmit={handleCreateUser} className="space-y-4 mt-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">First Name</Label><Input name="firstName" required className="bg-background/50" /></div>
                      <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Last Name</Label><Input name="lastName" required className="bg-background/50" /></div>
                    </div>
                    <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Email Coordinate</Label><Input name="email" type="email" required className="bg-background/50" /></div>
                    <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Temporary Password</Label><Input name="password" type="password" required className="bg-background/50" /></div>
        ...
        <DialogContent className="sm:max-w-[425px] border-border/50 bg-card/95 backdrop-blur-xl shadow-2xl">
          ...
          {userToEdit && (
            <form onSubmit={handleUpdateUser} className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">First Name</Label><Input name="firstName" defaultValue={userToEdit.firstName} required className="bg-background/50" /></div>
                <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Last Name</Label><Input name="lastName" defaultValue={userToEdit.lastName} required className="bg-background/50" /></div>
              </div>
              <div className="space-y-1.5 opacity-50 cursor-not-allowed"><Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Email Coordinate (Immutable)</Label><Input value={userToEdit.email} readOnly className="bg-background/50" /></div>
